...
class BaseDataModule(LightningDataModule):
    def __init__(self, hparams, mode, turn, data_aug = False, ):
        ...
        # captioning data nocaps | flickr
        # caption_ann = json.load(open(hparams.captioning_pth, "r"))
        # self.dataset["pseudo"] = self.get_dataset(pseudo_ann, mode=mode+"_pseudo")
        
        # no caps
        dataset_name = "nocaps"
        caption_pth =os.path.join(hparams.captioning_pth,dataset_name)
        file_list = os.listdir(caption_pth)
        file_path = os.path.join(caption_pth,file_list[0] )
        caption_ann = json.load(open(file_path, "r"))
        nocap_pseudo_data = self.get_dataset(caption_ann, mode=f"{dataset_name}_{mode}_pseudo")
        ...
        # file_list = os.listdir(caption_pth)
        # file_path = os.path.join(caption_pth,file_list[0] )
        # flickr_caption_ann = json.load(open(file_path, "r"))
        # flickr_pseudo_data = self.get_dataset(flickr_caption_ann, mode=f"{dataset_name}_{mode}_pseudo")
        
        pseudo_data = nocap_pseudo_data #+ flickr_pseudo_data
...
class VQAX_ST_DataModule(BaseDataModule):
    ...
    def get_dataset(self, anno, mode, data_aug=False):
        ...
        if os.path.exists(os.path.join(self.cfg.cached_dir, cached_filename)) and not data_aug:
            ...
            # Captioning dataset
            elif data_name == "nocaps" or data_name == "flickr":
