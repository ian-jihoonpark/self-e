...
def apply_mm_attention_rules(R_ss, R_qq, cam_sq, apply_normalization=True, apply_self_in_rule_10=True):
    ...
    R_sq_addition = torch.matmul(cam_sq, R_qq_normalized)
...
class Generator:
    def __init__(self, model, img_encoder):
        self.model = model
    ...
    def handle_self_attention_image(self, blocks, one_hot):
        for blk in blocks:
            ...
            if self.use_lrp:
                ...
                cam = blk.get_attn_probs().detach()
            # cam = avg_heads(cam, grad)
            cam = avg_heads(cam, None)
            self.R_i_i += torch.matmul(cam, self.R_i_i)
            
    def handle_co_attn_self_query(self, block, one_hot):
    ...
    def handle_co_attn_query(self, block, one_hot):
        if self.use_lrp:
            cam_q_i = block.crossattention.get_attn_cam().detach()
        else:
            cam_q_i = block.crossattention.get_attn().detach()
        # grad_q_i = block.crossattention.get_attn_gradients().detach()
        # cam_q_i = avg_heads(cam_q_i, grad_q_i)
        grad_q_i = torch.autograd.grad(one_hot, [block.crossattention.get_attn()], retain_graph=True)[0].detach()
        cam_q_i = avg_heads(cam_q_i, None)
    ...
    def generate_ours(self,btc_size, logit, index=None, use_lrp=False, normalize_self_attention=True, apply_self_in_rule_10=True):
        do_sample = False
        top_k =  0
        top_p =  0.9
        ...
        one_hot = F.one_hot(prev, num_classes=logit.shape[-1]).type(torch.float32)
        print(one_hot.shape)
        one_hot = one_hot.requires_grad_(True).cuda()
        one_hot = torch.sum(one_hot.cuda()*logit, dim=-1)
        self.model.zero_grad()
        # probas = logit.softmax(-1)[0, :, :-1]
        # keep = probas.max(-1).values > 0.9
        # one_hot = torch.zeros_like(logit).to(logit.device)
        # one_hot[0, keep, keep.nonzero()[0]] = 1
        # one_hot = torch.sum(one_hot.cuda() * logit)
        # one_hot.requires_grad_(True)
        # batch_size = btc_size
        ...
        decoder_blocks = self.model.transformer.h
        encoder_blocks = list(dict(self.image_encoder.encoder.visual.transformer.resblocks.named_children()).values())

        # initialize relevancy matrices
        # image_bboxes = encoder_blocks[0].self_attn.get_attn().shape[-1]
        image_bboxes = encoder_blocks[0].get_attn_probs().shape[-1]
        queries_num = decoder_blocks[0].attn.get_attn().shape[-1]
        # image self attention matrix
        self.R_i_i = torch.eye(image_bboxes, image_bboxes).to(encoder_blocks[0].get_attn_probs().device)
        # queries self attention matrix
        self.R_q_q = torch.eye(queries_num, queries_num).to(decoder_blocks[0].attn.get_attn().device)
        
        # impact of image boxes on queries
        self.R_q_i = torch.zeros(queries_num, image_bboxes-1).to(encoder_blocks[0].get_attn_probs().device)

        # image self attention in the encoder
        self.handle_self_attention_image(encoder_blocks, one_hot)

        # decoder self attention of queries followd by multi-modal attention
        for blk in decoder_blocks:
            # decoder self attention
            self.handle_co_attn_self_query(blk, one_hot)

            # encoder decoder attention
            self.handle_co_attn_query(blk, one_hot)
        aggregated = self.R_q_i.unsqueeze_(0)
